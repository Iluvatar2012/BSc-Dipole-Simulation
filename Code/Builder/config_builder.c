...
int read_build (char* infile) {
	...
	if (check != 0 && (check = fscanf(file, "tau 			%d\n", &tau)) < 1)
		check = 0;
	if (check != 0 && (check = fscanf(file, "dt 			%lf\n", &dt)) < 1)
		check = -1;
	if (check != 0 && (check = fscanf(file, "writestep		%d\n", &writestep)) < 1)
		check = -2;
	if (check != 0 && (check = fscanf(file, "D_rat 			%lf\n", &D_rat)) < 1)
		check = -3;
	...
	if (check != 0 && (check = fscanf(file, "Gamma 			%lf 		%lf	 		%lf\n", &G_min, &G_max, &G_step)) < 1)
		check = -5;
	if (check != 0 && (check = fscanf(file, "shear_gamma	%lf			%lf 		%lf\n", &g_min, &g_max, &g_step)) < 1)
		check = -6;

	// check whether all reads were successful
	if (check < 1) {
		fprintf(stderr, "Variables could not be correctly read from file \"%s,\" program will now terminate: %d \n", infile, check);
		return EXIT_FAILURE;
	}
