...
struct parameters *hdf5_read (char* file) {
	...
	hid_t	file_id, position_set, disp_set, tempset_id, psi4_set, psi6_set, lane_set;
	hid_t	position_space, disp_space, attr_write_id, attr_N_id;
	herr_t	status;
	...
	disp 			= malloc((steps+1)*2*N*sizeof(double));
	...
	status		= H5Dread(lane_set, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, laning);

	// close the rest of the file
	status 		= H5Dclose(psi4_set);
	status		= H5Dclose(psi6_set);
	status		= H5Dclose(lane_set);
